...
final class AddDataView: BaseVC, AddDataPresenterToViewProtocol {
    ...
    var presenter: AddDataViewToPresenterProtocol!
    ...
    override func setupBindings() {
        ...
        qrField
            ...
            .sink { [unowned self] value in
                self.presenter.showReaderModal()
            }.store(in: &disposables)
        ...
        addImagesButton
            ...
            .sink { [unowned self] value in
                self.presenter.showPhotoPickerModal(authorized: photoLibraryAuthorizationStatus == .authorized)
            }.store(in: &disposables)
    ...
    func observeInputtedFields(qrNumber: AnyPublisher<Int?, Never>,
        ...
        images
            .sink { [unowned self] value in
                self.setupSnapshot(images: value)
            }.store(in: &disposables)
    }
    ...
    private func setupSnapshot(images: [ImageModel]) {
...
extension AddDataView: UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
...
extension AddDataView: ImageCellDelegate {
    func onDelete(for model: ImageModel) {
        presenter.deleteImage(model: model)
    }
}
